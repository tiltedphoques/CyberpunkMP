...
Func HookVTable(T* pInstance, const size_t aIndex, Func aFunctionPtr)
{
    ...
    const ScopedContext memory(&pVTable[aIndex], sizeof(&pVTable[aIndex]));
...
#define TP_HOOK_IAT2(libraryName, functionName, hook) FunctionHookManager::GetInstance().Add(reinterpret_cast<void*>(&hook), libraryName, functionName)

#define TP_HOOK_COMMIT FunctionHookManager::GetInstance().InstallDelayedHooks();
