...
template <template <uint32_t, typename> class TRaw, uint32_t TOffset, typename TRawRet, typename... TRawArgs,
...
public:
    ...
    inline static void Enqueue(const std::function<void()> aFunc)
    {
        s_driver->Enqueue(aFunc);
    }
...
template<template<uint32_t, typename> class TRaw,
         uint32_t TOffset,
         typename TRet,
         typename... TArgs,
         typename TWrapper,
...
public:
    ...
    inline static TRet Handle(TArgs... aArgs)
    {
        ...
        {
            if constexpr (TFlow == HookFlow::MainThread)
            {
