...
void GenerateFunctionLibraries(const FunctionLibMap& map, bool isClient)
{
    for (auto& [key, val] : map)
    {
        std::ofstream o(GCyberpunkMpLocation / "Rpc" / (isClient ? "Client" : "Server") / (CsharpSanitizeName(key->name.ToString()) + ".cs"));
...
void RpcGenerator::DumpCsharp()
{
    ...
    {
        std::error_code ec;
        create_directory(GCyberpunkMpLocation / "Rpc", ec);
        create_directory(GCyberpunkMpLocation / "Rpc" / "Client", ec);
        create_directory(GCyberpunkMpLocation / "Rpc" / "Server", ec);

        std::ofstream o(GCyberpunkMpLocation / "Rpc" / "RedTypes.cs");
        o << "// This file is automatically generated\n\n";
        o << "#pragma warning disable CS8618\n";
        o << "#pragma warning disable CS0219\n\n";
        o << "using CyberpunkSdk;\n";
    ...
    GenerateFunctionLibraries(clientLibrary, true);
    GenerateFunctionLibraries(serverLibrary, false);
}
