...
// - 2023/2/16: Implemented TLS Initializer.
...
#include "ExeLoader.h"
#include "Launcher/steam/SteamCeg.h"
...
void ExeLoader::DecryptCeg(IMAGE_NT_HEADERS* apSourceNt)
{
    ...
    auto* sectionBase = IMAGE_FIRST_SECTION(apSourceNt);
    if (_strcmpi(reinterpret_cast<const char*>(sectionBase->Name), ".text"))
    {
        __debugbreak();
    }

    auto realEntry = steam::CrackCEGInPlace(GetOffset<uint8_t>(entry),
        {GetOffset<uint8_t>(sectionBase->VirtualAddress), sectionBase->SizeOfRawData});
...
bool ExeLoader::Load(const uint8_t* apProgramBuffer)
{
    ...
    const auto* targetTls = GetTargetRVA<IMAGE_TLS_DIRECTORY>(sourceNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS].VirtualAddress);
    PIMAGE_TLS_CALLBACK* tlsCallbacks = (PIMAGE_TLS_CALLBACK*)targetTls->AddressOfCallBacks;
    ...
    if (tlsCallbacks != NULL)
    {
        ...
        {
            tlsCallbacks[i](GetModuleHandle(NULL), DLL_PROCESS_ATTACH, NULL);
        }
    }
