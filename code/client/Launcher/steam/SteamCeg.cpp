...
uintptr_t CrackCEGInPlace(uint8_t* start, mem::region textSeg)
{
    ...
    std::memcpy(&stub, start - sizeof(SteamStubHeaderV31), sizeof(SteamStubHeaderV31));
    SteamXor(reinterpret_cast<uint8_t*>(&stub), sizeof(SteamStubHeaderV31), stub.XorKey);
    assert(stub.Signature == 0xC0DEC0DF);

    const bool NoEncryption = stub.Flags & 0x04 /*NoEncryption*/;
    if (!NoEncryption)
    {
        // decrypt IV in place
        ...
        uint8_t* pText = textSeg.start.as<uint8_t*>();
        std::memmove(pText + kCodeSize, pText, textSeg.size - kCodeSize);
        ...
        cbcDec.ProcessData(pText, pText, textSeg.size);
    }
